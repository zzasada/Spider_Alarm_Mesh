...
PROCESS_THREAD(main_proc, ev, data)
{
    ...
    while (1) {
        ...
        if (mira_fota_is_valid(0)) {
            printf("%s, Valid image: %ld bytes, version %d, seconds %lu s\n",
                net_state(), mira_fota_get_image_size(0), mira_fota_get_version(0), seconds);
        } else {
            printf("%s, No valid image available in cache %lu s\n",net_state(), seconds);
        }
    }
