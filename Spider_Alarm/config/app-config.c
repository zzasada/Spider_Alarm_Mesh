...
{
    printf("APP-CONFIG: Name: %s\n", app_config.name);
    printf("APP-CONFIG: Pan ID: %8lx\n", app_config.net_panid);
#if APP_CONFIG_EXPOSE_KEY
    printf("APP-CONFIG: Net key:");
    int i;
    for (i = 0; i < 16; i++){
    ...
    printf("\n");
#endif
    printf("APP-CONFIG: Rate: %d\n", (int) app_config.net_rate);
    printf("APP-CONFIG: Update interval: %d\n", (int) app_config.update_interval);
}
...
static uint8_t *hexstr(const uint8_t *data,int len)
{
...
static uint8_t *hexint(uint32_t data, int len)
{
    uint8_t buf[4];
    buf[0] = (data >> 24) & 0xff;
    buf[1] = (data >> 16) & 0xff;
...
static int dehexstr(void *tgt, const uint8_t *src, int len)
{
    int i;
    int nibble;
    uint8_t *tgt_i = tgt;
...
static int dehexint(uint32_t *tgt, const uint8_t *src, int len){
    *tgt = 0;
...
void config_nfc_on_open(mira_nfc_ndef_writer_t *writer){

    mira_nfc_ndef_write_copy(writer, MIRA_NFC_NDEF_TNF_MIME_TYPE, (const uint8_t *) "application/vnd.lumenradio.name", 31, NULL, 0, (uint8_t *) app_config.name, strlen(app_config.name));
#if APP_CONFIG_EXPOSE_KEY
    mira_nfc_ndef_write_copy(writer, MIRA_NFC_NDEF_TNF_MIME_TYPE, (const uint8_t *) "application/vnd.lumenradio.net_key", 34, NULL, 0, hexstr(app_config.net_key, 16), 32);
#endif
    mira_nfc_ndef_write_copy(writer, MIRA_NFC_NDEF_TNF_MIME_TYPE,(const uint8_t *) "application/vnd.lumenradio.net_panid", 36,NULL, 0, hexint(app_config.net_panid, 4), 8);
    mira_nfc_ndef_write_copy(writer, MIRA_NFC_NDEF_TNF_MIME_TYPE,(const uint8_t *) "application/vnd.lumenradio.net_rate", 35,NULL, 0,hexint(app_config.net_rate, 1), 2);
    mira_nfc_ndef_write_copy(writer, MIRA_NFC_NDEF_TNF_MIME_TYPE,(const uint8_t *) "application/vnd.lumenradio.update_interval", 42,NULL, 0,hexint(app_config.update_interval, 2), 4);
}

static int cmp_str(const char *match,const uint8_t *str,uint32_t len)
{
    if (strlen(match) != len){
        return 0;
    }
...
static void config_nfc_on_save(uint8_t *file, mira_size_t size)
{
    ...
    for (mira_nfc_ndef_iter_start(&iter, &rec, file, size); mira_nfc_ndef_iter_valid(&iter); mira_nfc_ndef_iter_next(&iter, &rec)){
        ...
        if (cmp_str("application/vnd.lumenradio.net_key", rec.type, rec.type_length)){
            ...
            {
                printf("APP-CONFIG: Invalid net_key\n");
                return;
            }
        }
        if (cmp_str("application/vnd.lumenradio.net_panid",rec.type, rec.type_length)){
            ...
            {
                printf("APP-CONFIG: Invalid net_panid \n");
                return;
            }
        ...
        if (cmp_str("application/vnd.lumenradio.net_rate",rec.type,rec.type_length))
        {
            ...
            {
                printf("APP-CONFIG: Invalid net_rate\n");
                return;
            }
        ...
        if (cmp_str("application/vnd.lumenradio.update_interval",rec.type,rec.type_length)){
            ...
            {
                printf("APP-CONFIG: Invalid update_interval\n");
                return;
            }
...
PROCESS_THREAD(app_config_writer, ev, data)
{
    ...
    while (1)
    {
        ...
        if(new_config_loaded) {
            printf("APP-CONFIG: Writing config, Pan ID: %08lx rate: %02x\n", new_config.net_panid, new_config.net_rate);

            status = mira_config_write(&new_config, sizeof(app_config_t));
            printf("APP-CONFIG: Status: %d\n", status);
            PROCESS_WAIT_WHILE(mira_config_is_working());

            printf("APP-CONFIG: Done\n");
            mira_sys_reset();
        }
    }
    PROCESS_END();
